...
class PredictorTrainer(Trainer):
    ...
    def log_step(self, step):
        ...
        with torch.no_grad():
            ...
            images = batch['images'].to(self.device)[:, :8]
            obs = obs[:, :8]
            action = action[:, :8]
            ...
            self.writer.add_video('compressed_video', torch.cat([images, compressed_video, compressed_video - images], dim=0), global_step=step)
            ...
            self.writer.add_video('predicted_video', torch.cat([images, predicted_video, predicted_video - images], dim=0), global_step=step)
            ...
            self.writer.add_video('generated_video_true_action', torch.cat([images, generated_video, generated_video - images], dim=0), global_step=step)
